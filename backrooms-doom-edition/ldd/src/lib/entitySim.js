...
import { ENTITY_PARAMS, makeSteer, stepActive } from './entities.js';
...
export function createEntitySim(world, opts = {}) {
  ...
  // dt ms. ctx {flashlight,recording,moving,sanity}. cb {onContact,onDocument,onAlert}
  function update(dt, player, ctx, cb) {
    ...
    for (let i = records.length - 1; i >= 0; i--) {
      ...
      if (activeSet.has(r)) {
        ...
        if (status === 'documented') { documented[r.type] = (documented[r.type] || 0) + 1; cb.onDocument && cb.onDocument(r.type); records.splice(i, 1); }
  ...
  return {
    ...
    records, documented, tierCounts,
    get count() { return records.length; },
    stats() { return { total: records.length, ...tierCounts }; },
    reset() { records.length = 0; for (const k in documented) delete documented[k]; clock = 0; spawnCd = 1500; },
  };
}
